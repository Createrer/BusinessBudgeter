...
public class CodeGenerator {
		...
		if (generateJsr303Annotations) {
			...
			if (StringUtils.isNotBlank(insertGrpClass)) {
				...
				for (String c : classes) {
					...
					// add only the class name and the FQ class name since the
					// import is added
			...
			for (String relationInfo : relations) {
				...
				if (domainClass.getName().equals(relationTokens[0])) {
					...
					if (domainRelations == null) {
						...
						domainClass.getRelations().put(relationTokens[0], domainRelations);// map
																							// key
																							// is
																							// the
																							// parent
					...
					if (dbRelations == null) {
						...
						dbClass.getRelations().put(relationTokens[0], dbRelations);// map
																					// key
																					// is
																					// the
																					// parent
					...
					if (repoRelations == null) {
						...
						repoClass.getRelations().put(relationTokens[0], repoRelations);// map
																						// key
																						// is
																						// the
																						// parent
	...
	public static void main(String[] args) {
		...
		generator.setPropertiesFile("resources/codegenerator.properties");
		generator.generate();
	}
